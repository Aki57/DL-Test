...
class linearRegressionModel:
  ...
  def constructModel(self):
    self.x = tf.placeholder(tf.float32, [None, self._x_dimen])
    self.y = tf.placeholder(tf.float32, [None, 1])
    self.w = self.weight_variable([self._x_dimen, 1])
    self.b = self.bias_variable([1])
    self.y_pred = tf.nn.bias_add(tf.matmul(self.x, self.w), self.b)

    mse = tf.reduce_mean(tf.squared_difference(self.y_pred, self.y))
  ...
  def train(self,x_train,y_train,x_test,y_test):
    ...
    for i in range(5000):
      ...
      self._sess.run(self.train_step, feed_dict = {self.x:batch[0], self.y:batch[1]})
      if i%10 == 0:
        train_loss = self._sess.run(self.loss, feed_dict = {self.x:batch[0], self.y:batch[1]})
  ...
  def predict(self, x_predict):
    ...
    for x_test_batch in self.predict_batch(x_predict, 100):
      pred = self._sess.run(self.y_pred, {self.x:x_test_batch})
